    ...
    <section className="bg-[color:var(--color-forest)] text-white py-20 px-6 text-center">
      <div className="max-w-3xl mx-auto animate-fade-up">
        ...
        <p className="text-lg opacity-90 text-[color:var(--color-cream-dark)] max-w-xl mx-auto">
          Tell us about your Loku Caters experience. Your feedback helps us keep improving and serving you the best way we can.
        </p>
      </div>
    </section>
  );
...
function ReviewSuccess({ onReset }: { onReset: () => void }) {
  return (
    <div className="bg-white p-12 rounded-3xl shadow-lg border border-[color:var(--color-border)] text-center animate-scale-in">
      ...
      <h2 className="text-2xl font-bold text-[color:var(--color-forest)] mb-4" style={{ fontFamily: "var(--font-serif)" }}>
        Thank You!
      </h2>
      <p className="text-[color:var(--color-muted)] mb-3 leading-relaxed">
        Your feedback is incredibly valuable to us. It helps us keep improving and serving you the best way we can.
      </p>
      <p className="text-[color:var(--color-muted)] mb-8 leading-relaxed text-sm">
        We appreciate you taking the time to share your thoughts.
      </p>
      <button onClick={onReset} className="bg-[color:var(--color-sage)] text-white px-8 py-3 rounded-full font-bold hover:bg-[color:var(--color-forest)] transition-colors">
...
function ratingLabel(rating: number): string {
  return ["", "Needs work", "Fair", "Good", "Great!", "Amazing!"][rating] ?? "";
}
    ...
    <div className="bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-[color:var(--color-border)] animate-fade-up">
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-5">
        <div className="flex flex-col gap-2">
          <label htmlFor="review-name" className="text-sm font-semibold text-[color:var(--color-text)]">
            Your Name <span className="font-normal text-[color:var(--color-muted)]">(optional)</span>
          </label>
          <input
            type="text"
            id="review-name"
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="review-message" className="text-sm font-semibold text-[color:var(--color-text)]">
            Your Review <span className="font-normal text-[color:var(--color-muted)]">(optional)</span>
          </label>
          <textarea
            id="review-message"
            value={message}
        ...
        <div className="flex flex-col gap-3">
          <label className="text-sm font-semibold text-[color:var(--color-text)]">Your Rating</label>
          <div className="flex items-center gap-4">
            <StarRating value={rating} onChange={onRatingChange} size={36} mode="input" />
            {rating > 0 && (
              <span className="text-sm font-medium animate-fade-in" style={{ color: "var(--color-accent)" }}>
                {ratingLabel(rating)}
              </span>
            )}
          </div>
...
function PublicReviews({ reviews }: { reviews: PublicReview[] }) {
  return (
    <CustomerReviewsSection reviews={reviews} className="pb-20 px-6" showTimestamp />
  );
}
    ...
    <main className="flex-1 bg-[color:var(--color-cream)]">
      ...
      <section className="py-16 px-6">
        <div className="max-w-xl mx-auto">
          {isSuccess ? (
            <ReviewSuccess onReset={resetReview} />
          ) : (
            <ReviewForm
              name={name}
              message={message}
