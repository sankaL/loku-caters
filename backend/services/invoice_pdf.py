...
def _styles() -> dict[str, ParagraphStyle]:
    return {
        "title": ParagraphStyle("invoice_title", fontName="Helvetica-Bold", fontSize=25, leading=28, textColor=FOREST, alignment=TA_RIGHT),
        "number": ParagraphStyle("invoice_number", fontName="Helvetica-Bold", fontSize=10, leading=13, textColor=SAGE, alignment=TA_RIGHT),
        "label": ParagraphStyle("invoice_label", fontName="Helvetica-Bold", fontSize=7, leading=9, textColor=SAGE, spaceAfter=3),
        "body": ParagraphStyle("invoice_body", fontName="Helvetica", fontSize=9, leading=12, textColor=TEXT),
        "body_bold": ParagraphStyle("invoice_body_bold", fontName="Helvetica-Bold", fontSize=9, leading=12, textColor=TEXT),
        "small": ParagraphStyle("invoice_small", fontName="Helvetica", fontSize=7.5, leading=10, textColor=MUTED),
        "table_head": ParagraphStyle("invoice_table_head", fontName="Helvetica-Bold", fontSize=8, leading=10, textColor=colors.white),
        "table_cell": ParagraphStyle("invoice_table_cell", fontName="Helvetica", fontSize=8, leading=10, textColor=TEXT),
        "table_right": ParagraphStyle("invoice_table_right", fontName="Helvetica", fontSize=8, leading=10, textColor=TEXT, alignment=TA_RIGHT),
        "total": ParagraphStyle("invoice_total", fontName="Helvetica-Bold", fontSize=11, leading=13, textColor=FOREST, alignment=TA_RIGHT),
    }
    ...
    *,
    ...
    header = Table(
        [
            [
                _logo(),
                Paragraph(f"<b>{_p(vendor.get('business_name') or 'Loku Caters')}</b>", styles["body_bold"]),
                [Paragraph("INVOICE", styles["title"]), Paragraph(_p(invoice_number), styles["number"])],
            ]
        ],
    ...
    vendor_lines = [vendor.get("business_address"), vendor.get("business_email"), vendor.get("business_phone")]
    customer_lines = [customer.get("email"), customer.get("phone")]
    from_block = [Paragraph("FROM", styles["label"]), Paragraph(_p(vendor.get("business_name")), styles["body_bold"])]
    from_block.extend(Paragraph(_p(line), styles["body"]) for line in vendor_lines if line)
    bill_block = [Paragraph("BILL TO", styles["label"]), Paragraph(_p(customer.get("name")), styles["body_bold"])]
    bill_block.extend(Paragraph(_p(line), styles["body"]) for line in customer_lines if line)
    meta_rows = [
    ...
    blocks.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP"), ("BOX", (0, 0), (-1, -1), 0.6, BORDER), ("INNERGRID", (0, 0), (-1, -1), 0.4, BORDER), ("BACKGROUND", (0, 0), (-1, -1), colors.white), ("LEFTPADDING", (0, 0), (-1, -1), 12), ("RIGHTPADDING", (0, 0), (-1, -1), 12), ("TOPPADDING", (0, 0), (-1, -1), 10), ("BOTTOMPADDING", (0, 0), (-1, -1), 10)]))
    ...
    for line in order.get("lines") or []:
        rows.append([
            ...
            Paragraph(_money(line.get("unit_price"), currency), styles["table_right"]),
            Paragraph(_money(line.get("subtotal"), currency), styles["table_right"]),
        ])
    items = Table(rows, colWidths=[3.26 * inch, 0.60 * inch, 1.45 * inch, 1.55 * inch], repeatRows=1)
    item_style = [("BACKGROUND", (0, 0), (-1, 0), FOREST), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("LEFTPADDING", (0, 0), (-1, -1), 8), ("RIGHTPADDING", (0, 0), (-1, -1), 8), ("TOPPADDING", (0, 0), (-1, 0), 8), ("BOTTOMPADDING", (0, 0), (-1, 0), 8), ("TOPPADDING", (0, 1), (-1, -1), 7), ("BOTTOMPADDING", (0, 1), (-1, -1), 7), ("LINEBELOW", (0, 1), (-1, -1), 0.4, BORDER)]
    ...
    note_parts: list[Any] = []
    if invoice.get("memo"):
        note_parts.extend([Paragraph("MEMO", styles["label"]), Paragraph(_p(invoice.get("memo")), styles["body"]), Spacer(1, 10)])
    if not paid and vendor.get("payment_method") != "none":
        payment_text = {"etransfer": "E-transfer", "cash": "Cash", "other": "Other"}.get(vendor.get("payment_method"), "Payment")
        ...
        note_parts.extend([Paragraph("PAYMENT", styles["label"]), Paragraph(payment_text, styles["body"])])
    if note_parts:
        payment_box = Table([[note_parts]], colWidths=[6.5 * inch])
        payment_box.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), CREAM), ("BOX", (0, 0), (-1, -1), 0.5, BORDER), ("LEFTPADDING", (0, 0), (-1, -1), 12), ("RIGHTPADDING", (0, 0), (-1, -1), 12), ("TOPPADDING", (0, 0), (-1, -1), 10), ("BOTTOMPADDING", (0, 0), (-1, -1), 10)]))
    ...
    def add_page_number(canvas, document) -> None:
        ...
        canvas.drawRightString(letter[0] - 0.62 * inch, 0.3 * inch, f"Page {document.page}")
        canvas.restoreState()
